 ...
 */
angular.module("panzoom", ["monospaced.mousewheel"]).directive("panzoom", ["$document", "PanZoomService", function ($document, PanZoomService) {
    var api = {};
    var viewportHeight;
    var viewportWidth;
    return {
        ...
        controller: ["$scope", "$element", function ($scope, $element) {
            ...
            $scope.config.friction = $scope.config.friction !== undefined ? $scope.config.friction : 10;
            $scope.config.haltSpeed = $scope.config.haltSpeed !== undefined ? $scope.config.haltSpeed : 100;
            $scope.config.scalePerZoomLevel = $scope.config.scalePerZoomLevel !== undefined ? $scope.config.scalePerZoomLevel : 2;
            $scope.config.zoomStepDuration = $scope.config.zoomStepDuration !== undefined ? $scope.config.zoomStepDuration : .2;
            $scope.config.zoomToFitZoomLevelFactor = $scope.config.zoomToFitZoomLevelFactor !== undefined ? $scope.config.zoomToFitZoomLevelFactor : .95;
            $scope.config.zoomButtonIncrement = $scope.config.zoomButtonIncrement !== undefined ? $scope.config.zoomButtonIncrement : 1;
            ...
            $scope.config.keepInBoundsRestoreForce = $scope.config.keepInBoundsRestoreForce !== undefined ? $scope.config.keepInBoundsRestoreForce : .5;
            $scope.config.keepInBoundsDragPullback = $scope.config.keepInBoundsDragPullback !== undefined ? $scope.config.keepInBoundsDragPullback : .7;
            ...
            if ($scope.config.initialZoomToFit) {
                ...
                $scope.base = {
                    ...
                    pan: {x: $scope.config.initialPanX, y: $scope.config.initialPanY}
                }
            }
            $scope.model.zoomLevel = $scope.base.zoomLevel;
            $scope.model.isPanning = false;
            $scope.model.pan = {x: $scope.base.pan.x, y: $scope.base.pan.y};
            ...
            var getCenterPoint = function () {
                var center = {x: frameElement.width() / 2, y: frameElement.height() / 2};
                return center
            };
            var changeZoomLevel = function (newZoomLevel, clickPoint, duration) {
                ...
                var translationFromZoom = function (zoomLevel) {
                    ...
                    var t1 = {x: pmark.x - s1 / s0 * (pmark.x - t0.x), y: pmark.y - s1 / s0 * (pmark.y - t0.y)};
                    return {x: t1.x - t0.x, y: t1.y - t0.y}
                };
                $scope.zoomAnimation = {
                    deltaZoomLevel: deltaZoomLevel,
                    translationFromZoom: translationFromZoom,
                    duration: duration,
                    progress: $scope.config.disableZoomAnimation ? 1 : 0
                }
            };
            var zoomIn = function (clickPoint) {
                changeZoomLevel($scope.base.zoomLevel + $scope.config.zoomButtonIncrement, clickPoint)
            };
            ...
            var getModelPosition = function (viewPosition) {
                ...
                var s = getCssScale($scope.base.zoomLevel);
                var t = $scope.base.pan;
                return {x: 1 / s * (pmark.x - t.x), y: 1 / s * (pmark.y - t.y)}
            };
            ...
            var AnimationTick = function () {
                ...
                return function () {
                    var now = jQuery.now();
                    var deltaTime = lastTick ? (now - lastTick) / 1e3 : 0;
                    lastTick = now;
                    if ($scope.zoomAnimation) {
                        $scope.zoomAnimation.progress += deltaTime / $scope.zoomAnimation.duration;
                        if ($scope.zoomAnimation.progress >= 1) {
                            $scope.zoomAnimation.progress = 1;
                            syncModelToDOM();
                            $scope.base.zoomLevel = $scope.model.zoomLevel;
                            $scope.base.pan.x = $scope.model.pan.x;
                            $scope.base.pan.y = $scope.model.pan.y;
            ...
            $scope.onTouchStart = function ($event) {
                ...
                if ($event.originalEvent.touches.length === 1) {
                    ...
                    previousPosition = {length: x * x + y * y}
                }
            };
            $scope.onTouchMove = function ($event) {
                ...
                if ($event.originalEvent.touches.length === 1) {
                    ...
                    var clickPoint = {x: centerX - frameElement.offset().left, y: centerY - frameElement.offset().top};
                    changeZoomLevel($scope.base.zoomLevel + delta * 1e-4, clickPoint);
                    previousPosition = {length: length}
                }
            };
            ...
            $scope.onMousemove = function ($event) {
                ...
                var now = jQuery.now();
                var timeSinceLastMouseEvent = (now - lastMouseEventTime) / 1e3;
            ...
            $scope.onMouseup = function () {
                ...
                if ($scope.panVelocity) {
                    var initialMultiplier = Math.max(0, Math.pow(timeSinceLastMouseEvent + 1, -4) - .2);
...
angular.module("panzoomwidget", []).directive("panzoomwidget", ["$document", "PanZoomService", function ($document, PanZoomService) {
    ...
    return {
        ...
        compile: function compile() {
            return {
                pre: function preLink() {
                    ...
                    if (!panzoomId) {
                        throw"Error in setup. You must define attribute panzoom-id on the <panzoomwidget> element in order to link it to the " + "id of the <panzoom> element. Ref: "
                    }
                    PanZoomService.getAPI(panzoomId).then(function (api) {
                        ...
                        scope.widgetConfig = {zoomLevelHeight: 10};
        ...
        template: '<div class="panzoomwidget">' + '<div ng-click="zoomIn()" ng-mouseenter="zoomToLevelIfDragging(config.zoomLevels - 1)" class="zoom-button zoom-button-in">+</div>' + '<div class="zoom-slider" ng-mousedown="onMousedown()" ' + 'ng-click="onClick($event)">' + "<div class=\"zoom-slider-widget\" ng-style=\"{'height': widgetConfig.zoomLevelHeight - 2 +'px'}\"></div>" + '<div ng-repeat="zoomLevel in getZoomLevels()" ' + " class=\"zoom-level zoom-level-{{zoomLevel}}\" ng-style=\"{'height': widgetConfig.zoomLevelHeight +'px'}\"></div>" + "</div>" + '<div ng-click="zoomOut()" ng-mouseenter="zoomToLevelIfDragging(0)" class="zoom-button zoom-button-out">-</div>' + "<div ng-transclude></div>" + "</div>",
        replace: true
    }
}]);
angular.module("panzoom").factory("PanZoomService", ["$q", function ($q) {
    var panZoomAPIs = {};
    ...
    return {registerAPI: registerAPI, unregisterAPI: unregisterAPI, getAPI: getAPI}
}]);
